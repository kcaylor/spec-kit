...
- Fix `/speckit.taskstoepic` helper to surface prerequisite errors when JSON output is empty.
...
- Prevent `[specify]` warnings from polluting JSON output in prerequisite checks.

## [0.0.31] - 2026-02-07

- Add `--prefix` support to `/speckit.taskstoepic` and task parser.
...
- Add `--template-repo`, `--template-overlay-repo`, and `--template-overlay-path` flags to `specify init`.
